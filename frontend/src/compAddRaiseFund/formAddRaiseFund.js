...
function CompFormAddRF() {
  ...
  const onSubmitHandler = useMutation(async (event) => {
    ...
    try {
      ...
      console.log(typeof formData.get("goal"));

      const response = await api.post("/fund", config);
      console.log("responseAddRF", response);
